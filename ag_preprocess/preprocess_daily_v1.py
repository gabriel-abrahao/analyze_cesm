...
for scen in scens:
    ...
    #     r = re.compile(invarname+"_day_"+model+"_"+scen+".*")
    #     varfiles = [infolder + f for f in allfiles if r.match(f)]
    #     if len(varfiles) == 1:
    #         varfname = varfiles[0]
    #     else:
    #         print("ERROR: More than one file matches the pattern")
    #         print(r)
    #         print(allfiles)

    #     invards = xr.open_dataset(varfname)
    #     # invards = xr.open_mfdataset(varfiles, combine = "by_coords")
    #     # invards = invards.drop(["time_bnds","lat_bnds","lon_bnds"])


    #     # Try removing metadata that might not be consistent
    #     # such as bounds and "height" coordinate attributes
    #     try:
    #         del(invards.time.attrs["bounds"])
    #     except: None
    #     try:
    #         del(invards.lat.attrs["bounds"])
    #     except: None
    #     try:
    #         del(invards.lon.attrs["bounds"])
    #     except: None
    #     try:
    #         del(invards[invarname].attrs["coordinates"])
    #     except: None         
    #     try: 
    #         invards = invards.drop(["time_bnds","lat_bnds","lon_bnds"])
    #     except: None      
    ...
    #     # Rename
    #     invards = invards.rename({invarname:varnamedict[invarname]})

    #     bigds = xr.merge([bigds,invards])

    # # Get global attributes from the last DataSet, and remove variable_id
    # bigds.attrs = invards.attrs
    # del(bigds.attrs["variable_id"])
