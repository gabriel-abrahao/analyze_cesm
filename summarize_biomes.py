...
# inpvname = "TREFHT"
inpvnames = ['CLDTOT', 'FSDS', 'PRECT', 'PS', 'QREFHT', 'TREFHT', 'TREFHTMN', 'TREFHTMX', 'U10']
# inpvnames = ['CLDTOT', 'FSDS', 'PRECT', 'PS']
...
outfname = "output_small/allvars_biomes_monthly.nc"
...
# Number of times in a Dask chunk
ntimechunks = 10
...
for item in alldata:
    ...
    # item['inp'] = xr.open_dataset(item['inpfname'])[['CLDTOT','PRECT']].sel(lat = slice(minlat,maxlat), lon = slice(minlon,maxlon))
    item['inp'] = xr.open_dataset(item['inpfname'], chunks = {'time':ntimechunks})[inpvnames].sel(lat = slice(minlat,maxlat), lon = slice(minlon,maxlon))

# poi = xr.open_dataset(item['inpfname'], chunks = {'time':1})
# repr([i for i in poi.data_vars])
...
print("Adding singleton dimensions...")
for item in alldata:
...
print("Merging...")
allinp = xr.merge([item['inp'] for item in alldata])
